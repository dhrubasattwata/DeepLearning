...
def L_model_forward(X, parameters):
    ...
    caches = []
    A = X
    L = len(parameters) // 2                  # number of layers in the neural network: W's + b's

    # Implement [LINEAR -> RELU]*(L-1). Add "cache" to the "caches" list.
    for l in range(1, L):                              # remember Lth is the last hidden layer, hence till L; post this sigmoid
        A_prev = A
        ### START CODE HERE ### (≈ 2 lines of code)
...
def compute_cost(AL, Y):
    ...
    ### START CODE HERE ### (≈ 1 lines of code) # sum over multiply over Y times log
...
import inspect
lines = inspect.getsource(sigmoid_backward)
print(lines)
...
def L_model_backward(AL, Y, caches):
    ...
    current_cache = caches[-1]  # accessing last cache
...
def update_parameters(parameters, grads, learning_rate):
    ...
    L = len(parameters) // 2 # number of layers in the neural network: parameters has W's and b's
