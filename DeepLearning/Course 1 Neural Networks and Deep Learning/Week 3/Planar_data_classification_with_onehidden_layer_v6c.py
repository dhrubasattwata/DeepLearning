...
def forward_propagation(X, parameters):
    ...
    ### START CODE HERE ### (≈ 4 lines of code)  # we have defined the functions aboves
    W1 = parameters["W1"]
    b1 = parameters["b1"]
    W2 = parameters["W2"]
    b2 = parameters["b2"]
    ...
    A1 = np.tanh(Z1)
    Z2 = np.dot(W2,A1) + b2
    A2 = sigmoid(Z2)  # check sigmoid function from previous week
    ### END CODE HERE ###
...
def predict(parameters, X):
    ...
    ### START CODE HERE ### (≈ 2 lines of code)
    A2, cache = forward_propagation(X, parameters)
    predictions = np.round(A2)  # being cheeky here. Should be a if loop
